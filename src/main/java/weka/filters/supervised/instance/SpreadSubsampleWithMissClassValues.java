 ...
 * @version $Revision$
 **/
public class SpreadSubsampleWithMissClassValues extends Filter implements SupervisedFilter, OptionHandler {
	...
	*/
	public boolean isDeleteInstancesWithMissingClass() {
		return this.isDeleteInstancesWithMissingClass;
	}

	/**
	 * @param isDeleteInstancesWithMissingClass the isDeleteInstancesWithMissingClass to set
	 */
	public void setDeleteInstancesWithMissingClass(final boolean isDeleteInstancesWithMissingClass) {
	...
	* @throws InterruptedException
	...
	private void createSubsample() throws InterruptedException {
		...
		this.getInputFormat().sort(classI); // Instances with missing values for the attribute are placed at the end of the dataset
				...
				do {
					...
					if (t.get("" + index) == null) {
						...
						if (index >= 0) {
							...
							this.push(newInst); // No need to copy instance
						}
					}
				} while (!ok);
			}
		...
		if (!this.isDeleteInstancesWithMissingClass) {
			for (int i = 0; i < this.getInputFormat().numInstances(); i++) {
				Instance current = this.getInputFormat().instance(i);
	...
	private int[] getClassIndices() {
		...
		int firstMissingClassI = this.getInputFormat().numInstances(); // if no missing class value, the firstMissingClassI equls getInputFormat().numInstances()
		for (int i = 0; i < this.getInputFormat().numInstances(); i++) {
			Instance current = this.getInputFormat().instance(i);
